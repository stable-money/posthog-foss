...
def rerank_chunks(
    ...
    *,
    ...
    """
    Returns the input order (RRF order from ``search_knowledge``) trimmed to ``top_k``.

    LLM reranking is unavailable in this build; the original ranking is always used.
    """
    if not results:
        return []
